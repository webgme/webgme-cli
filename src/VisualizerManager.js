...
VisualizerManager.prototype._addToVisualizersJSON = function (
  id,
  title,
  panelPath
) {
...
// Overrides
VisualizerManager.prototype._getJsonForConfig = function (
  installInfo,
  callback
) {
    ...
    if (!result.secondary) {
      ...
      fs.stat(visJsonPath, function (e, file) {
        if (e) {
          if (e.code === "ENOENT") {
            utils.mkdir(visDir);
            return fs.writeFile(visJsonPath, "[]", (err) =>
              callback(err, result)
            );
          } else {
