...
var createCommands = function (components, defArgs, descTs, opts) {
  ...
  components.forEach((component) => {
    descT = _.template(
      customDescs.hasOwnProperty(component.name)
        ? customDescs[component.name]
        : descTs
    );
    args = customArgs.hasOwnProperty(component.name)
      ? customArgs[component.name]
      : defArgs;
...
var createSubCommands = function (dir, defArgs, descTs, opts) {
  ...
  if (!componentNames[component]) {
    ...
    if (inferrable && config) {
      // Try to infer the component. Assume name is provided
      var name = process.argv[2],
        nameIndex = 2,
        aliased = Object.keys(alias).map((a) => alias[a]);

      while (process.argv[nameIndex] && process.argv[nameIndex][0] === "-") {
        // skip options
        nameIndex++;
      }
      ...
      if (component) {
        ...
        component = [component, component.replace(/s$/, "")].find(
          (c) => componentNames[c] || aliased.find((a) => c === a)
        );
      }
