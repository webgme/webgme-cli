...
describe("Viz tests", function () {
  ...
  describe("new viz", function () {
    ...
    it("should add panelPaths", function () {
      ...
      const localPanelPath = config.visualization.panelPaths.find((panelPath) =>
        panelPath.includes("src/visualizers/panels")
      );
    ...
    it("should add Visualizers.json to config.webgme.js", function () {
      console.log(
        fse.readFileSync(path.join(PROJECT_DIR, WebGMEConfig), "utf8")
      );
      const config = require(path.join(PROJECT_DIR, WebGMEConfig));
      const localVisJson = config.visualization.visualizerDescriptors.find(
        (desc) => desc.includes("./src/visualizers/Visualizers.json")
      );
  ...
  describe("import viz", function () {
    ...
    describe("projects created with webgme-setup-tool", function () {
      ...
      before(function (done) {
        this.timeout(5000);
        process.chdir(PROJECT_DIR);
        manager.import({ name: OTHER_VIZ, project: otherProject }, function () {
          utils.requireReload(
            path.join(PROJECT_DIR, WebGMEConfig),
            CONFIG_PATH,
            path.join(VizBasePath, "Visualizers.json"),
