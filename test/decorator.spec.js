...
describe("Decorator tests", function () {
  ...
  var RAW_DECORATOR_NAME = "MyNew",
    ...
    DECORATOR_SRC = path.join(
      DecoratorBasePath,
      DECORATOR_NAME,
      DECORATOR_NAME + ".js"
    ),
    DECORATOR_TEST = path.join(
      PROJECT_DIR,
      "test",
      "decorators",
      DECORATOR_NAME,
      DECORATOR_NAME + ".spec.js"
    );
  ...
  describe("import decorator", function () {
    ...
    describe("projects NOT created with webgme-setup-tool", function () {
      ...
      before(function (done) {
        ...
        (CONFIG_PATH = path.join(PROJECT_DIR, CONFIG_NAME)),
          utils.getCleanProject(PROJECT_DIR, function () {
            ...
            manager.import(
              { name: otherDecorator, project: otherProject },
              done
            );
          });
      });
      ...
      it("should add the project to the " + CONFIG_NAME, function () {
        ...
        assert.notEqual(
          config.dependencies.decorators[otherDecorator],
          undefined
        );
      });
    ...
    describe("projects created with webgme-setup-tool", function () {
      before(function (done) {
        ...
        process.chdir(PROJECT_DIR);
        manager.import(
          { name: OTHER_DECORATOR, project: otherProject },
          function () {
            utils.requireReload(path.join(PROJECT_DIR, "package.json"));
            done();
          }
      ...
      it("should add the project to the " + CONFIG_NAME, function () {
        var configText = fse.readFileSync(CONFIG_PATH),
          config = JSON.parse(configText);
        assert.notEqual(
          config.dependencies.decorators[OTHER_DECORATOR],
          undefined
        );
      });
