...
define([
  ...
  "js/logger",
  "text!./templates/EnabledLayout.html",
  "text!./EnabledLayoutConfig.json",
], function (_jQueryLayout, Logger, defaultLayoutTemplate, LayoutConfigJSON) {
  "use strict";
  ...
  var CONFIG = JSON.parse(LayoutConfigJSON),
    ...
    SPACING_OPEN = WebGMEGlobal.SUPPORTS_TOUCH
      ? SPACING_OPEN_TOUCH
      : SPACING_OPEN_DESKTOP,
    SPACING_CLOSED = WebGMEGlobal.SUPPORTS_TOUCH
      ? SPACING_CLOSED_TOUCH
      : SPACING_CLOSED_DESKTOP,
    SIDE_PANEL_WIDTH = 202;

  var EnabledLayout = function (params) {
    this._logger =
      (params && params.logger) ||
      Logger.create(
        "gme:Layouts:EnabledLayout",
   ...
   * @return {undefined}
   */
  EnabledLayout.prototype.init = function () {
    var self = this;
   ...
   * @return {undefined}
   */
  EnabledLayout.prototype.addToContainer = function (panel, container) {
    if (container === "header") {
   ...
   * @return {undefined}
   */
  EnabledLayout.prototype.remove = function (panel) {
   ...
   * @return {undefined}
   */
  EnabledLayout.prototype.destroy = function () {
    this._body.empty();
  };
  ...
  EnabledLayout.prototype._onCenterResize = function () {
    if (this._canvas) {
      this._canvas.setSize(
        this._centerPanel.width(),
        this._centerPanel.height()
      );
    }
  };

  EnabledLayout.prototype._onToolboxResize = function () {
    if (this._toolbox) {
      this._toolbox.setSize(
        this._toolboxPanel.width(),
        this._toolboxPanel.height()
      );
    }
  };
